...
function category(req, res) {
  ...
  const sql = 'SELECT p.name AS Prodotto, p.image AS Immagine, p.price AS Prezzo, name_category AS Categoria FROM products AS p JOIN categories ON categories.id = p.category_id WHERE name_category = ? LIMIT 6';
...
function search(req, res) {
  ...
  const sql = 'SELECT p.*, c.name_category, b.name_brand FROM products p JOIN categories c ON c.id = p.category_id JOIN brands b ON b.id = p.brand_id WHERE p.name LIKE ?';

  connection.query(sql, [searchMethod], (err, results) => {
    if (err)
      return res.status(500).json({
